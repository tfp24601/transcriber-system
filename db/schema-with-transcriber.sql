...
CREATE TABLE IF NOT EXISTS transcriber.recordings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS transcriber.transcripts (
...
CREATE TABLE IF NOT EXISTS transcriber.jobs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    recording_id UUID NOT NULL REFERENCES transcriber.recordings(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_recordings_user_created ON transcriber.recordings(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_jobs_recording_id ON transcriber.jobs(recording_id);
CREATE INDEX IF NOT EXISTS idx_jobs_status ON transcriber.jobs(status);

-- Updated_at trigger function
CREATE OR REPLACE FUNCTION transcriber.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS update_users_updated_at ON transcriber.users;
CREATE TRIGGER update_users_updated_at 
    BEFORE UPDATE ON transcriber.users 
    FOR EACH ROW EXECUTE FUNCTION transcriber.update_updated_at_column();

DROP TRIGGER IF EXISTS update_recordings_updated_at ON transcriber.recordings;
CREATE TRIGGER update_recordings_updated_at 
    BEFORE UPDATE ON transcriber.recordings 
    FOR EACH ROW EXECUTE FUNCTION transcriber.update_updated_at_column();

DROP TRIGGER IF EXISTS update_transcripts_updated_at ON transcriber.transcripts;
CREATE TRIGGER update_transcripts_updated_at 
    BEFORE UPDATE ON transcriber.transcripts 
    FOR EACH ROW EXECUTE FUNCTION transcriber.update_updated_at_column();

DROP TRIGGER IF EXISTS update_jobs_updated_at ON transcriber.jobs;
CREATE TRIGGER update_jobs_updated_at 
    BEFORE UPDATE ON transcriber.jobs 
    FOR EACH ROW EXECUTE FUNCTION transcriber.update_updated_at_column();
...
CREATE OR REPLACE FUNCTION transcriber.cleanup_old_recordings(days_to_keep INTEGER DEFAULT 90)
RETURNS TABLE (
...
BEGIN
    ...
    DELETE FROM transcriber.transcripts 
    WHERE recording_id IN (
        SELECT id FROM transcriber.recordings 
        WHERE created_at < NOW() - INTERVAL '1 day' * days_to_keep
    );
    GET DIAGNOSTICS trans_count = ROW_COUNT;
    
    -- Delete old recordings
    DELETE FROM transcriber.recordings 
    WHERE created_at < NOW() - INTERVAL '1 day' * days_to_keep;
...
CREATE OR REPLACE VIEW transcriber.user_storage_stats AS
SELECT 
    u.id,
    u.email,
    u.display_name,
...
FROM transcriber.users u
LEFT JOIN transcriber.recordings r ON u.id = r.user_id
